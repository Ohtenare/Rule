...
export default async function(ctx) {
  // 0. 环境变量配置
  const HIDE_IP = (ctx.env.HIDE_IP || "true").trim() !== "false";
  ...
  // --- 辅助与解析函数 ---
  const maskIP = (ip) => {
    if (!HIDE_IP || !ip || ip === "获取失败") return ip;
    if (ip.includes(':')) { // IPv6
    ...
    children: [
      ...
      { type: 'stack', direction: 'row', gap: 10, children: [
          ...
          { type: 'stack', direction: 'column', gap: 4.5, flex: 1, children: [
              ...
              Row("globe.asia.australia.fill", C.cpu, "公网", maskIP(localData.ip), C.text), // 应用打码
          ...
          { type: 'stack', direction: 'column', gap: 4.5, flex: 1, children: [
              Row("paperplane.fill", C.mem, "出口", maskIP(proxyData.ip), C.text), // 应用打码
